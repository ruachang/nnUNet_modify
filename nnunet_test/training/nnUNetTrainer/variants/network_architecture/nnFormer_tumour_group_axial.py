...
class AxialAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, proj_drop=0.):
        ...
        self.proj = nn.Linear(dim, dim)
        self.proj_drop = nn.Dropout(proj_drop)
        self.axialAttention = AxialImageTransformer(dim, heads=num_heads, dim_index=1, depth=dim // num_heads)
    def forward(self, x, mask=None,pos_embed=None):
        B_, N, C = x.shape
        #qkv = self.qkv(x)
        #qkv=qkv.reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
        
        x = self.axialAttention(x)
        if pos_embed is not None:
            x = x+pos_embed
        x = self.proj(x)
        x = self.proj_drop(x)
...
class AxialTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        self.attn = AxialAttention(
            dim=dim, num_heads=num_heads, window_size = to_3tuple(self.window_size))
