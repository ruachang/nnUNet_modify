...
# * Spatial attention module
    def __init__(self, input_channel, group_number, kernel_size=7):
        super(SpatialAttention, self).__init__()

        self.conv1 = nn.Conv2d(input_channel, input_channel / group_number, kernel_size, padding=kernel_size//2, bias=False)
...
class Bottleneck(nn.Module):
    def __init__(self,
        ...
        self.embed_dim = embed_dim
        self.out_indices = out_indices
        num_layers = len(ori_depths)
        self.pos_drop = nn.Dropout(p=drop_rate)

        # stochastic depth
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(ori_depths))]  # stochastic depth decay rule
        # remaining encoder
        # build layers
        self.layers = nn.ModuleList()
        for i in range(2):
            i_layer = num_layers - 2 + i
            layer = BasicLayer(
                dim=int(embed_dim * 2 ** (i_layer)),
                input_resolution=(
                    pretrain_img_size[0] // patch_size[0] // 2 ** i_layer, pretrain_img_size[1] // patch_size[1] // 2 ** i_layer,
                ...
                attn_drop=attn_drop_rate,
                drop_path=dpr[sum(
                    ori_depths[:i_layer]):sum(ori_depths[:i_layer + 1])],
                norm_layer=norm_layer,
                downsample=PatchMerging
                # the last downsample is deleted(move in the bottle_neck)
                if (i == 0) else None
        ...
        num_features = [int(embed_dim * 2 ** i) for i in range(num_layers - 2, num_layers)]
        self.encoder_num_features = num_features
        ...
        layer = BasicLayer_up(
            ...
            attn_drop=attn_drop_rate,
            drop_path=dpr[sum(
                ori_depths[::-1][1:][:0]):sum(ori_depths[::-1][1:][:1])],
            norm_layer=norm_layer,
            upsample=Patch_Expanding
            )
        self.layers.append(layer)
        self.decoder_num_features = [int(embed_dim * 2 ** (len(ori_depths) - 2))]
    def forward(self,x, Ws, Wh, Ww):
        # Ws, Wh, Ww = x.size(2), x.size(3), x.size(4)
        
        # x is the next input; x_out is the recorded input
        # First layer bottle neck: global attention + down sample
        x_out, S, H, W, x, Ws, Wh, Ww = self.layers[0](x, Ws, Wh, Ww)
        norm_layer = getattr(self, f'norm{0}')
        x_out = norm_layer(x_out)
        skips = x_out.view(-1, S, H, W, self.encoder_num_features[0]).permute(0, 4, 1, 2, 3).contiguous()
        
        # second layer bottle neck: global attention
        x_out, S, H, W, x, Ws, Wh, Ww = self.layers[1](x, Ws, Wh, Ww)
        norm_layer = getattr(self, f'norm{1}')
        x_out = norm_layer(x_out)
        neck = x_out.view(-1, S, H, W, self.encoder_num_features[1]).permute(0, 4, 1, 2, 3).contiguous()
        S, H, W = neck.size(2), neck.size(3), neck.size(4)
        # The third layer bottle neck: upsample + global attention
        neck = neck.flatten(2).transpose(1, 2).contiguous()
        skips = skips.flatten(2).transpose(1, 2).contiguous()
        neck = self.pos_drop(neck)
...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        if self.do_ds:
            ...
            group_split_out = [group_decoder_out[j][2 - 1] for j in range(self.group_num)]
            seg_outputs.append(self.final[-(len(group_decoder_out[0]) + 1)](group_split_out))
            return seg_outputs[-1]
