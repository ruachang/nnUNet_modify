...
from nnunetv2.training.nnUNetTrainer.variants.network_architecture.nnFormer_tumor import nnFormer
...
sys.path.append("/root/Desktop/nnUNet")
from nnunet_test.training.nnUNetTrainer.variants.network_architecture.nnFormer_tumor import nnFormer
from nnunet_test.training.nnUNetTrainer.variants.network_architecture.nnFormer_tumour_group import nnFormer
PRETRAINED_MODEL_PATH = "F:\dataset\mindBoggle\nnFormer\pretrain_weight-20231201T182650Z-001.zip"
class nnFormerTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        # deep supervision setting for the nnFormer
        self.deep_supervision_scales = None
        self.ds_loss_weights = None
        # * predefined netork structure in nnFormer_tumor.py
        ...
        self.num_classes = None # -> self.initialize
        self.crop_size = None  # -> self.initialize
    ...
    def initialize(self):
        if not self.was_initialized:
            # only use the channel_name's info to set the channels fo the net
            self.num_input_channels = determine_num_input_channels(self.plans_manager, self.configuration_manager,
                                                                   self.dataset_json)
            self.pool_op_kernel_sizes = self.configuration_manager.pool_op_kernel_sizes  
            self.num_classes = self.label_manager.num_segmentation_heads
            self.crop_size = self.configuration_manager.patch_size
            self.network = self.build_network_architecture(self, self.plans_manager, self.dataset_json,
    ...
    # a class that can use without instance, but can not visit the prperty
    @staticmethod
    def build_network_architecture(self, plans_manager: PlansManager,
        ...
        # ! maybe delete it because already have model load outside
        ...
        #     self.network.cuda()
        self.network.inference_apply_nonlin = softmax_helper_dim1  
        return self.network
    def _get_deep_supervision_scales(self):
        self.deep_supervision_scales = [[1, 1, 1]] + list(list(i) for i in 1 / np.cumprod(
                    np.vstack(self.pool_op_kernel_sizes[1:4]), axis=0))[:-1]
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        if dim == 2:
            ...
            # ! suppose that don't do any elastic deform anymore
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        flops, params = profile(self.network, inputs=(data, ))
        print(flops, params)
        # Autocast is a little bitch.
        # If the device_type is 'cpu' then it's slow as heck and needs to be disabled.
        # If the device_type is 'mps' then it will complain that mps is not implemented, even if enabled=False is set. Whyyyyyyy. (this is why we don't make use of enabled=False)
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            #     flops, params = profile(self.network, inputs=(data.unsqueeze(0), ))
            #     print(flops, params)
